...
urlpatterns = patterns(
    ...
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/object_assignments/$',
        'object_assignments'),
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/subject_assignment/(?P<assign_uuid>[\w-]+)/$',
        'subject_assignment'),
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/object_assignment/(?P<assign_uuid>[\w-]+)/$',
        'object_assignment'),
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/rules/$',
        'rules'),
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/rule/$',
        'rule'),
    url(r'^intra-extension/(?P<uuid>[\w-]{32,36})/rule/(?P<rule_uuid>[\w-]+)/$',
        'rule'),
)